function phaser1(filename,speed,mix,feedback)
%% Phaser 1 model
% This model reads in a mono .wav file and writes a processed version to a
% new file called "phaser1.wav", saved in current MATLAB directory. 
% It uses four first order allpass filters in the phase shifting stage.
%
% Input arguments:
...
%% Initialization
% read in the audio file and extract the sampling frequency
...
baseLFO = (baseLFO+1)/2;
% define min and max values for allpass filters' break frequency, in Hz
fbmin = 139.48;
fbmax = 1760.84;
% map the break frequency values to the LFO
LFO = fbmin + (fbmax-fbmin)*baseLFO;
%% Highpass filter 1
...
% preassign a vector to store the allpass chain break frequency for each filter
frameFreq = zeros(1,numberFramesInput);
...
for i=1:numberFramesInput
    ...
    frameFreq(i) = LFO(frameCentre(i));
    % process each frame with the 4 allpass filter cascade
    outputFrame = allpassCascade(currentFrame,frameFreq(i),fs,FB);
...
audiowrite('phaser1.wav',output,fs);
end